...
def score(results):
    ...
    return sum(results[k]["success_rate"] for k in SCORE_SETS if k in results)


def gain_in_se(new, old):
    ...
    var = sum(r[k]["success_rate"] * (1 - r[k]["success_rate"]) / r[k]["n"]
              for r in (new, old) for k in SCORE_SETS if k in r)
    return (score(new) - score(old)) / max(var, 1e-12) ** 0.5
    ...
    with EnvPool(args.workers) as pool:
        ...
        for r in range(first, args.rounds + 1):
            ...
            if (Path(args.root) / version / "manifest.json").exists():     # killed after freezing
                ...
                seeds = [s for s in range(DAGGER_SEED_BASE + 1000 * r, DAGGER_SEED_BASE + 1000 * r + args.episodes)
