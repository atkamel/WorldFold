...
The exact per-index definition is [imitation.md §2.3](imitation.md#23-index-map-and-who-consumes-what-phase-5b);
this is the short version. Figures are regenerated by `imitation.viz.report_figures` into
[reports/media/](reports/media/): `obs_layout.json` (the block table), `episode_trace.json`
(one episode over time), `obs_heatmap.png`, `cams_panel.png` and `dr_grid.png`.

**Per control step (20 Hz), the env produces:**

| stream | shape | what it is | privileged? |
|---|---|---|---|
| `state` | float32 [139] | both arms (joint pos/vel, gripper command, EE pose + velocity, grasp flag) 0-49; cloth (corners, corner velocities, 9 vertices, CoM, height stats, corner-to-goal) 50-118; task (progress, goals, per-corner progress, time left, stage, settle) 119-138 | arms: sensor-available except the 2 grasp flags (24, 49); everything from 50 on is privileged |
| `main` | uint8 [3,128,128] | fixed overhead-oblique camera at (0.274, 0, 1.172), fovy 45° | sensor |
| `left_wrist_cam`, `right_wrist_cam` | uint8 [3,64,64] | cameras on each gripper, fovy 75° | sensor |
| action | float32 [12] in [-1, 1] | per arm: 5 joint deltas (× 0.05 rad) + gripper command | — |
| labels | scalars | reward, fold_score, success, failure code, terminated/truncated | never an input |

**Why 139 and not 141.** The base env emits 141 values. M1.1 removes its 4-dim task one-hot
(constant for a single task) and appends 2 phase signals, the expert's stage and settle counter:
141 − 4 + 2 = 139 (`imitation.spec.OBS_DIM`).

**"Proprio" means the 48 sensor-available arm dims** (0-49 minus the grasp flags 24 and 49).
That is everything a real robot measures about itself.

**Who consumes what:**

| consumer | inputs |
|---|---|
| scripted expert (teacher) | full sim state (not the vector) |
| state policies (BC, DAgger, diffusion, IQL actor) | `state` 139 × 2-step history |
| vision policy | the 3 cameras + the 48 proprio dims (the other 91 are masked to zero) |
| policy teacher (distillation) | `state` 139 × 2, inside the rollout worker |
| success detector | `main` camera only |
| IQL critic | `state` 139 × 2 + an 8 × 12 macro-action |

Every camera frame is stored beside its episode (`<episode>.img.npz`) and hashed into the
dataset version, so a re-render gets a new version.
